    ...
    <div class="modal-card">
      ...
      <div class="second">
        ...
        <div class="sensorInfo" v-if="sensorClick">
          <div v-if="modifyMode && !addMode">
            ...
            <div style='width: 100%;'>
              <div style='width: 50%; height: 100%; float:left; text-align: center;'>
                <button style="text-align: center" v-if="modifyMode" class="delButton" @click="delSensor">
                  센서 삭제
                </button>
              </div>
              <div style='width: 50%; height: 100%; float:right; text-align: center;'>
                <button style="text-align: center" v-if="modifyMode" class="saveButton" @click="editEvent">
                  센서 저장
                </button>
              </div>
          ...
          <p v-if="!modifyMode" style="padding-bottom: 5px; border-bottom :2px solid gray; width:90%">
            {{ currentSensor.SENSOR_NAME }}
          </p>
          <p v-if="!modifyMode" style="margin-top:15px; margin-bottom:3px; font-size: 17px">
        ...
        <div v-if="!imgChk" style="margin-top: 30px">
          <div v-if="!outMode">
            <c-button
              ...
              @click=";[loadSensors(0), buttonClick(0), loadHumanIcon(1,1), drawSensorBound()]"
            ></c-button>
            <li
...
export default {
  ...
  computed: {
    ...
    ...mapState('store/store', ['storeInfo']),
    ...
    ...mapGetters('sensor/sensor', ['GET_SELECT_SENSOR_EVENT']),
  },
  data() {
    return {
      center: [750, 750],
  ...
  methods: {
      ...
      if (this.modifyMode === true) {
        if (this.addMode === true) {
          ...
          if (this.clickSensorIndex !== null) {
            this.sensors[this.clickSensorIndex].setIcon(this.targetIcons.BLUE_ICON)
          }
          this.addMode = true
    ...
    iconSettings() {
      ...
      this.eventIcons = {
        SEVEN_ICON : new L.Icon({
          iconUrl: require('~/assets/images/temp/7store_ico.png'),
          iconSize: [40, 40],
          iconAnchor: [12, 41],
          popupAnchor: [1, -34],
        }),
        E_MART_ICON: new L.Icon({
          iconUrl: require('~/assets/images/temp/2store_ico.png'),
          iconSize: [40, 40],
          iconAnchor: [12, 41],
          popupAnchor: [1, -34],
        }),
        GS25_ICON: new L.Icon({
          iconUrl: require('~/assets/images/temp/GS25_ico.png'),
          iconSize: [40, 40],
          iconAnchor: [12, 41],
          popupAnchor: [1, -34],
        }),
        CU_ICON: new L.Icon({
          iconUrl: require('~/assets/images/temp/cu_ico.png'),
          iconSize: [40, 40],
          iconAnchor: [12, 41],
          popupAnchor: [1, -34],
        }),
        VIPS_ICON: new L.Icon({
          iconUrl: require('~/assets/images/temp/vips_ico.png'),
          iconSize: [40, 40],
          iconAnchor: [12, 41],
          popupAnchor: [1, -34],
        }),
        ONE_ICON: new L.Icon({
          iconUrl: require('~/assets/images/temp/granmam_ico.png'),
          iconSize: [40, 40],
          iconAnchor: [12, 41],
          popupAnchor: [1, -34],
        })
    ...
    loadHumanIcon(lon, lat) {
      ...
      let cnt = 1
      let idx = 10
      for (const [key, value] of Object.entries(this.eventIcons)) {
        console.log(`${key}: ${value}`);
        L.marker([lon + (cnt*idx), lat+ (cnt*idx)], {
          icon: value,
          visible: true,
          draggable: true,
          name: 'human',
    ...
    outSensorDraw() {
      ...
      this.sensorList.forEach((sensor) => {
        ...
        if (
          this.divideInfo.TARGET_DV_CD === sensor.SENSOR_TARGET_DV_CD &&
          this.divideInfo.TARGET_ID === sensor.SENSOR_TARGET_ID
        ) {
    ...
    drawSensorBound() {
      ...
      senlist.forEach((sensor) => {
        ...
        ).bindPopup(contents, { autoClose: false, autoPan: false }).addTo(this.layerBoundGroup)
        //.openPopup()
        idx = idx + 1
      })
    },
